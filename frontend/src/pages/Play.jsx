...
const Play = () => {
  ...
  const { emit, on, isConnected, queueStats } = useSocket();
  ...
  useEffect(() => {
    if (mode === 'team_ranked' || mode === 'team_unranked') {
      navigate('/in-progress?feature=team-duel', { replace: true });
      return;
    }
    if (mode === 'private') {
      navigate('/in-progress?feature=private-match', { replace: true });
    }
  }, [mode, navigate]);
  ...
  const handleStartQueueMatch = (queueType) => {
    ...
    setIsSearching(true);
    emit('matchmaking:join', { queueType });
  };
  ...
  const handleStartAI = async () => {
    ...
    try {
      ...
      const { gameCode } = response.data.data.game;
      ...
      <div className={styles.play}>
        ...
        <div className={styles.modeGrid}>
          <Card hoverable className={styles.modeCard} onClick={() => setMode('ranked')}>
            <div className={styles.modeIcon} style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)' }}>
              <FiUsers size={32} />
            </div>
            <h2>Ranked Match</h2>
          ...
          <Card hoverable className={styles.modeCard} onClick={() => setMode('unranked')}>
            <div className={styles.modeIcon} style={{ background: 'linear-gradient(135deg, #0ea5e9, #22d3ee)' }}>
              <FiUsers size={32} />
            </div>
            <h2>Unranked 1v1</h2>
          ...
          <Card hoverable className={styles.modeCard} onClick={() => setMode('ai')}>
            <div className={styles.modeIcon} style={{ background: 'linear-gradient(135deg, #06b6d4, #10b981)' }}>
              <FiCpu size={32} />
            </div>
            <h2>vs AI</h2>
          ...
          <Card hoverable className={styles.modeCard} onClick={() => navigate('/in-progress?feature=team-duel')}>
            <div className={styles.modeIcon} style={{ background: 'linear-gradient(135deg, #f97316, #ef4444)' }}>
              <FiTool size={32} />
            </div>
          ...
          <Card hoverable className={styles.modeCard} onClick={() => navigate('/in-progress?feature=team-duel')}>
            <div className={styles.modeIcon} style={{ background: 'linear-gradient(135deg, #f59e0b, #f97316)' }}>
              <FiTool size={32} />
            </div>
          ...
          <Card hoverable className={styles.modeCard} onClick={() => navigate('/in-progress?feature=private-match')}>
            <div className={styles.modeIcon} style={{ background: 'linear-gradient(135deg, #f59e0b, #ef4444)' }}>
              <FiTool size={32} />
            </div>
            <h2>Private Match</h2>
            <p>This mode is temporarily disabled while being rebuilt.</p>
      ...
      <div className={styles.play}>
        ...
        <Card className={styles.matchmakingCard}>
          {isSearching ? (
            ...
            <div className={styles.readyToSearch}>
              <div className={styles.modeIcon} style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)' }}>
                <FiUsers size={40} />
              </div>
              ...
              <Button
                variant="primary"
                size="lg"
                onClick={() => handleStartQueueMatch(queueConfig.queueType)}
        ...
        <Card className={styles.aiCard}>
          ...
          <div className={styles.difficultyGrid}>
            {['easy', 'medium', 'hard', 'expert'].map((diff) => (
              <button
                key={diff}
                className={`${styles.difficultyOption} ${aiDifficulty === diff ? styles.selected : ''}`}
