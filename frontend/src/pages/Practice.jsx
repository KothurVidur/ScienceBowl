...
const Practice = () => {
  ...
  const [practiceMode, setPracticeMode] = useState('question');
  const [scope, setScope] = useState('tossup');
  ...
  const updateDifficultyMin = nextMin => {
    ...
    setDifficultyRange(prev => {
      ...
      return {
        min: clampedMin,
        max: Math.max(clampedMin, prev.max)
      };
    });
  };
  const updateDifficultyMax = nextMax => {
    ...
    setDifficultyRange(prev => {
      ...
      return {
        min: Math.min(prev.min, clampedMax),
        max: clampedMax
      };
    });
  };
  ...
  useEffect(() => {
    ...
    const questionReadMs = question.type === 'tossup' ? questionWords * WORD_REVEAL_PACE_MS : 0;
    let optionsReadMs = 0;
  ...
  }, [question, result, loading, practiceMode, timeLeft, timerStartAt, typingStartedAt, lastProgressAt, hardCapAt, answer]);
  useEffect(() => {
    const onKeyDown = e => {
      if (loading) return;
      if (!question) return;
  ...
  return <div className={styles.practice}>
      <header className={styles.header}>
        ...
        <div className={styles.sessionStats}>
          ...
          {stats.total > 0 && <span className={styles.accuracy}>{Math.round(stats.correct / stats.total * 100)}%</span>}
        </div>
      </header>
      ...
      <section className={styles.filters}>
        <div className={styles.filterGroup}>
          ...
          <div className={styles.filterOptions}>
            <button className={`${styles.filterOption} ${practiceMode === 'question' ? styles.active : ''}`} onClick={() => setPracticeMode('question')}>

              Question Mode
            </button>
            <button className={`${styles.filterOption} ${practiceMode === 'reading' ? styles.active : ''}`} onClick={() => setPracticeMode('reading')}>
        ...
        <div className={styles.filterGroup}>
          <label>Scope</label>
          <div className={styles.filterOptions}>
            {['tossup', 'bonus', 'cycle'].map(type => <button key={type} className={`${styles.filterOption} ${scope === type ? styles.active : ''}`} onClick={() => setScope(type)}>
        ...
        <div className={styles.filterGroup}>
          ...
          <div className={styles.filterOptions}>
            <button className={`${styles.filterOption} ${selectedCategories.length === 0 ? styles.active : ''}`} onClick={() => setSelectedCategories([])}>

              All
            </button>
            {categories.map(cat => <button key={cat.id} className={`${styles.filterOption} ${selectedCategories.includes(cat.id) ? styles.active : ''}`} onClick={() => toggleCategory(cat.id)} style={{
          ...
          <div className={styles.difficultyRangeWrap}>
            <div className={styles.difficultyRangeSliders}>
              <div className={styles.rangeRow}>
                <span>Min</span>
                <input type="range" min="0" max="1" step="0.01" value={difficultyRange.min} onChange={e => updateDifficultyMin(e.target.value)} className={styles.timerSlider} />

                <span>{Number(difficultyRange.min).toFixed(2)}</span>
              </div>
              <div className={styles.rangeRow}>
                <span>Max</span>
                <input type="range" min="0" max="1" step="0.01" value={difficultyRange.max} onChange={e => updateDifficultyMax(e.target.value)} className={styles.timerSlider} />
        ...
        <div className={styles.filterGroup}>
          <label>Timer</label>
          {practiceMode === 'question' && <div className={styles.filterOptions}>
              <button className={`${styles.filterOption} ${!timerEnabled ? styles.active : ''}`} onClick={() => setTimerEnabled(false)}>

                Unlimited
              </button>
              <button className={`${styles.filterOption} ${timerEnabled ? styles.active : ''}`} onClick={() => setTimerEnabled(true)}>

                Timed
              </button>
            </div>}
          {(practiceMode === 'reading' || timerEnabled) && <div className={styles.timerSliderWrap}>
              <input type="range" min="1" max="60" step="1" value={timeLimitSeconds} onChange={e => setTimeLimitSeconds(Number(e.target.value))} className={styles.timerSlider} disabled={practiceMode === 'reading'} />

              <span className={styles.timerValue}>
                {practiceMode === 'reading' ? question?.type === 'bonus' ? '20s' : '2s' : `${timeLimitSeconds}s`}
              </span>
            </div>}
        </div>
      </section>
      ...
      <section className={styles.questionArea}>
        {loading ? <Card className={styles.startCard}>
            <div className={styles.loadingSpinner}></div>
            <h2>Loading Question...</h2>
          </Card> : !question ? <Card className={styles.startCard}>
            <FiFilter size={48} />
            <h2>Ready to Practice?</h2>
            <p>Select your filters and start practicing</p>
            <Button variant="primary" size="lg" onClick={fetchQuestion} loading={loading} icon={<FiRefreshCw />}>

              Get Question
            </Button>
          </Card> : <Card className={styles.questionCard}>
          ...
          backgroundColor: getCategoryColor(question.category)
            ...
            <div className={styles.timerBadge} data-critical={timeLeft !== null && Number(timeLeft) <= 5}>

              <FiClock />
              <span>{timeLeft === null ? 'Unlimited' : `${timeLeft}s`}</span>
            </div>

            {practiceMode === 'reading' && !result && <div className={styles.stallBadge} data-critical={Number(stallSecondsLeft || 0) <= 0.8}>
            ...
            {question.format === 'mc' && question.choices && <div className={styles.choices}>
                {Object.entries(question.choices).map(([letter]) => <div key={letter} className={styles.choiceLine}>
                    {visibleChoices[letter] || `${letter}. ...`}
                  </div>)}
              </div>}

            {!result && <div className={styles.answerInput}>
                <input type="text" value={answer} onChange={handleAnswerInputChange} placeholder={question.format === 'mc' ? 'Type W/X/Y/Z or the exact option text...' : 'Type your answer...'} autoFocus />

              </div>}

            {result && <div className={`${styles.result} ${result.isCorrect ? styles.correct : styles.incorrect}`}>
                {result.isCorrect ? <>
                    <FiCheck size={24} />
                    <span>
            ...
            <div className={styles.actionsRow}>
              {!result ? <>
                  ...
                  <div className={`${styles.actionsSlot} ${styles.centerSlot}`}>
                    <Button variant="success" onClick={handleSubmit} disabled={!String(answer || '').trim()} icon={<FiCheck />}>

                      Submit Answer
                    </Button>
                  </div>
                  <div className={`${styles.actionsSlot} ${styles.rightSlot}`}>
                    <Button variant="ghost" size="sm" onClick={handleReportQuestion} disabled={!question?.id || reportingQuestionId === String(question.id)} icon={<FiFlag />}>
                  ...
                  <div className={`${styles.actionsSlot} ${styles.leftSlot}`}>
                    {!result.isCorrect && <Button variant="secondary" onClick={handleWasRight}>
                        I Was Right
                      </Button>}
                    {result.isCorrect && <Button variant="secondary" onClick={handleWasWrong}>
                        I Was Wrong
                      </Button>}
                  </div>
                  <div className={`${styles.actionsSlot} ${styles.centerSlot}`}>
                    {result.canAdvanceToBonus ? <Button variant="primary" onClick={goToBonusInCycle} icon={<FiRefreshCw />}>

                        Go To Bonus
                      </Button> : <Button variant="primary" onClick={fetchQuestion} loading={loading} icon={<FiRefreshCw />}>
