...
const Home = () => {
  ...
  const [counter, setCounter] = useState({ questions: 0, matches: 0, players: 0 });

  useEffect(() => {
    const start = Date.now();
    const durationMs = 1400;
    const targets = { questions: 10000, matches: 250000, players: 24000 };
  ...
  const categories = [
    { name: 'Mathematics', icon: <FiHash />, color: '#7c3aed' },
    { name: 'Physics', icon: <FiAperture />, color: '#2563eb' },
    { name: 'Chemistry', icon: <FiDroplet />, color: '#f59e0b' },
    { name: 'Biology', icon: <FiActivity />, color: '#10b981' },
    { name: 'Earth and Space', icon: <FiGlobe />, color: '#0ea5e9' },
    { name: 'Energy', icon: <FiBatteryCharging />, color: '#f97316' }
  ];
      ...
      <section className={styles.section}>
        ...
        <div className={styles.categoryGrid}>
          {categories.map((category) => (
            <article key={category.name} className={styles.categoryCard} style={{ '--cat-color': category.color }}>
      ...
      <section className={styles.section}>
        ...
        <div className={styles.trustGrid}>
          {testimonials.map((item) => (
            <blockquote key={item.by} className={styles.quoteCard}>
              <p>{item.quote}</p>
              <cite>{item.by}</cite>
            </blockquote>
          ))}
