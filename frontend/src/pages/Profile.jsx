...
import { FiActivity, FiAward, FiCalendar, FiClock, FiCpu, FiGlobe, FiHash, FiPercent, FiTarget, FiTrendingUp, FiZap } from 'react-icons/fi';
...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      ...
      try {
        ...
        const [profileRes, statsRes, gamesRes, historyRes] = await Promise.all([userAPI.getProfile(username), userAPI.getStats(username), userAPI.getGames(username, {
          page: 1,
          limit: selectedGamesRange.limit
  ...
  useEffect(() => {
    ...
    const fetchGames = async () => {
      ...
      try {
        const selectedGamesRange = gamesRangeOptions.find(range => range.id === gamesRange) || gamesRangeOptions[0];
        const gamesRes = await userAPI.getGames(username, {
          page: 1,
          limit: selectedGamesRange.limit
        });
        if (isCancelled) return;
  ...
  return <div className={styles.profile}>
      <header className={styles.hero}>
        ...
        <div className={styles.ratingCard}>
          ...
          color: getRankColor(profile.rating)
        }}>
            {profile.rating}
          </span>
      ...
      <Card className={styles.chartCard}>
        <div className={styles.sectionHeader}>
          ...
          <div className={styles.sectionTabs}>
            {ratingRangeOptions.map(option => <button key={option.id} className={`${styles.sectionTab} ${ratingRange === option.id ? styles.activeSectionTab : ''}`} onClick={() => setRatingRange(option.id)}>
        ...
        {loadingRatingHistory ? <div className={styles.inlineLoader}>
            <div className={styles.inlineSpinner}></div>
            <p>Updating rating history...</p>
          </div> : ratingHistory.length > 1 ? <div className={styles.chart}>
            <ResponsiveContainer width="100%" height={230}>
              <LineChart data={ratingHistory.map(entry => ({
            ...
            rating: entry.rating
              ...
              boxShadow: '0 10px 30px rgba(1, 8, 24, 0.55)'
                ...
                <Line type="monotone" dataKey="rating" stroke="#2563eb" strokeWidth={3} dot={{
              fill: '#2563eb',
              r: 3
            }} activeDot={{
              r: 5,
          ...
          </div> : <div className={styles.emptyPanel}>Not enough games yet to render a chart.</div>}
      </Card>
      ...
      <Card className={styles.categoryCard}>
        ...
        <div className={styles.categoryGrid}>
          {canonicalCategoryRows.map(({
          key,
          label,
          color,
          icon
        }) => {
          const data = categoryStats[key] || {
            answered: 0,
            correct: 0
          };
          const accuracy = data.answered > 0 ? Math.round(data.correct / data.answered * 100) : 0;
          return <div key={key} className={styles.categoryItem}>
      ...
      <Card className={styles.gamesCard}>
        ...
        <div className={styles.gamesFilterBar}>
          <div className={`${styles.sectionTabs} ${styles.gamesFilterGroupLeft}`}>
            {gamesRangeOptions.map(option => <button key={option.id} className={`${styles.sectionTab} ${gamesRange === option.id ? styles.activeSectionTab : ''}`} onClick={() => setGamesRange(option.id)}>
          ...
          <div className={`${styles.sectionTabs} ${styles.gamesFilterGroupMiddle}`}>
            {ratedFilterOptions.map(option => <button key={option.id} className={`${styles.sectionTab} ${ratedFilter === option.id ? styles.activeSectionTab : ''}`} onClick={() => setRatedFilter(option.id)}>
        ...
        {loadingGames ? <div className={styles.inlineLoader}>
            <div className={styles.inlineSpinner}></div>
            <p>Updating recent games...</p>
          </div> : filteredGames.length > 0 ? <div className={styles.gamesList}>
          ...
          return <Link key={game._id} to={`/games/${game._id}/review?from=profile&u=${encodeURIComponent(username)}`} state={{
            fromProfile: true,
            profileUsername: username
                  ...
                  <article className={`${styles.gameItem} ${won ? styles.won : game.winner === 'tie' ? styles.tie : styles.lost}`}>
                    ...
                    {typeof ratingChange === 'number' && <div className={`${styles.ratingChange} ${ratingChange >= 0 ? styles.positive : styles.negative}`}>
                        {ratingChange >= 0 ? '+' : ''}
                        {ratingChange}
                      </div>}
                  </article>
          ...
          </div> : <div className={styles.emptyPanel}>No games match these filters.</div>}
      </Card>
    </div>;
};
