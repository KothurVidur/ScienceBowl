...
const GameReview = () => {
  ...
  const handleLeaveReview = async (targetPath) => {
    try {
      setIsLeaving(true);
      // No manual protest voting in review anymore; unresolved protests are async.
    } catch (err) {
  ...
  const winnerName = game.winner === 'player1'
    ...
    : game.winner === 'player2'
      ...
      : game.winner === 'tie'
        ? 'Tie'
        : (game.winner || 'N/A');
  const playAgainPath = String(game.gameType || '') === 'ranked'
    ? '/play?mode=ranked&autoQueue=1'
    : String(game.gameType || '') === 'unranked_1v1'
    ...
    <div className={styles.reviewPage}>
      <header className={styles.header}>
        ...
        <div className={styles.actions}>
          {canPlayAgain && (
            <Button variant="primary" onClick={() => handleLeaveReview(playAgainPath)} disabled={isLeaving}>
              {isLeaving ? 'Leaving...' : 'Play Again'}
            </Button>
          )}
          {profileReturnPath && (
            <Button variant="secondary" onClick={() => handleLeaveReview(profileReturnPath)} icon={<FiArrowLeft />} disabled={isLeaving}>
              Back to Profile
            </Button>
          )}
      ...
      <section className={styles.questionList}>
        {cycles.map((cycle) => {
                ...
                <div className={styles.questionHeader}>
                  ...
                  <div className={styles.questionMeta}>
                    ...
                    <button
                      ...
                      onClick={() => handleReportQuestion(reportId)}
                      disabled={!reportId || isReporting}
          ...
          return (
            <Card key={`cycle-${cycle.cycle}`} className={styles.cycleCard}>
