...
const normalizeType = (value) => {
  ...
  if (text === 'TOSSUP') return 'TOSSUP';
  if (text === 'BONUS') return 'BONUS';
  if (text === 'TOSS-UP') return 'TOSSUP';
...
const normalizeFormat = (value) => {
  ...
  if (text === 'multiple choice' || text === 'multiplechoice' || text === 'mc') return 'Multiple Choice';
...
const difficultyFilterFromInput = (input) => {
  ...
  values.forEach((value) => {
    ...
    if (lower === 'easy') {
      clauses.push({ difficulty: { $gte: 0, $lt: 0.33 } });
      return;
    }
    if (lower === 'medium') {
      clauses.push({ difficulty: { $gte: 0.33, $lt: 0.66 } });
      return;
    }
    if (lower === 'hard') {
      clauses.push({ difficulty: { $gte: 0.66, $lte: 1 } });
      return;
    }

    const numeric = Number(value);
    if (Number.isFinite(numeric)) {
      clauses.push({ difficulty: clampDifficulty(numeric) });
    }
  });
...
const normalizeQuestionPayload = (input = {}) => {
  const canonicalAnswer = input?.answer?.canonical;
  const alternates = Array.isArray(input?.answer?.alternates)
    ? input.answer.alternates
    : [];
  ...
  return {
    ...
    choices: input.choices && typeof input.choices === 'object' ? input.choices : undefined,
    answer: {
      canonical: String(canonicalAnswer || '').trim(),
