...
const answerSchema = new mongoose.Schema({
  ...
  alternates: [{ type: String, trim: true }]
}, { _id: false });
...
const questionSchema = new mongoose.Schema({
  ...
  difficulty: { type: Number, min: 0, max: 1, required: true, default: 0.5 },
  ...
  relatedTossup: { type: mongoose.Schema.Types.ObjectId, ref: 'Question' }
}, {
  timestamps: true
});
...
questionSchema.pre('validate', function normalizeQuestion(next) {
  try {
    ...
    const alternates = getAlternateAnswers(this.answer)
      ...
      .filter(Boolean);
    const uniqueAlternates = Array.from(new Set(alternates.map((alt) => alt.toLowerCase())))
      .map((lower) => alternates.find((alt) => alt.toLowerCase() === lower));

    this.answer = { canonical, alternates: uniqueAlternates };
...
questionSchema.index({ type: 1, format: 1 });
questionSchema.index({ 'source.packet': 1, 'source.round': 1 });
...
questionSchema.methods.checkAnswer = function checkAnswer(userAnswer) {
  ...
  if (isMC) {
    ...
    const canonicalLetter = /^[WXYZ]$/i.test(String(canonicalAnswer || '').trim())
      ? String(canonicalAnswer).trim().toUpperCase()
      : null;

    let resolvedLetter = canonicalLetter;
    if (!resolvedLetter && this.choices) {
      resolvedLetter = ['W', 'X', 'Y', 'Z'].find((letter) =>
        normalizeAnswerText(this.choices?.[letter] || '') === normalizedCanonicalAnswer
      ) || null;
    }
...
questionSchema.statics.getRandomQuestions = async function getRandomQuestions(count = 10, options = {}) {
  const query = { isActive: true, type: 'TOSSUP' };
...
questionSchema.statics.getBalancedQuestions = async function getBalancedQuestions(count = 10) {
  ...
  for (const category of QUESTION_CATEGORIES) {
    const categoryQuestions = await this.aggregate([
      { $match: { isActive: true, type: 'TOSSUP', category } },
...
questionSchema.statics.getTossupBonusCycles = async function getTossupBonusCycles(cycles = 10, options = {}) {
  ...
  const difficultyMatch = { difficulty: { $gte: difficultyMin, $lte: difficultyMax } };
