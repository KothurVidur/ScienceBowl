...
class RatingService {
  ...
  getActivityMultiplier(player) {
    ...
    let multiplier = 1;
    if (gamesPlayed < 15) multiplier += 0.25;else if (gamesPlayed < 40) multiplier += 0.1;
    if (inactiveDays >= 180) multiplier += 0.25;else if (inactiveDays >= 60) multiplier += 0.15;else if (inactiveDays >= 30) multiplier += 0.08;
    if (gamesPlayed >= 150 && inactiveDays < 14) multiplier -= 0.12;else if (gamesPlayed >= 80 && inactiveDays < 14) multiplier -= 0.06;
  ...
  async processGameResult(game) {
    ...
    const player2K = player2 ? Math.round(player2BaseK * player2ActivityMultiplier * marginMultiplier) : this.BASE_K;
    const player1Change = isTie ? 0 : this.calculateRatingChange(player1Rating, player2Rating, player1Score, player1K);
    const player2Change = player2 ? isTie ? 0 : this.calculateRatingChange(player2Rating, player1Rating, player2Score, player2K) : 0;
    player1.rating = Math.max(this.RATING_FLOOR, player1Rating + player1Change);
    player1.ratingHistory.push({
  ...
  async updatePlayerQuestionStats(userId, category, isCorrect, responseTime) {
    ...
    if (catKey === 'other' && !user.stats.categoryStats.other) {
      user.stats.categoryStats.other = {
        answered: 0,
        correct: 0
      };
    }
