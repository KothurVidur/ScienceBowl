...
const sendPasswordResetEmail = async ({ toEmail, username, resetUrl }) => {
  if (!nodemailer) return { sent: false, reason: 'nodemailer dependency missing' };
  if (!isSmtpConfigured()) return { sent: false, reason: 'smtp not configured' };

  const transporter = createTransporter();
  if (!transporter) return { sent: false, reason: 'smtp transport unavailable' };

  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 560px; margin: 0 auto; line-height: 1.5;">
      <h2 style="margin-bottom: 12px;">Reset your Science Bowl Online password</h2>
      ...
      <p style="color:#6b7280;font-size:12px;">Science Bowl Online</p>
    </div>
  `;
  ...
  try {
    await transporter.sendMail({
      ...
      to: toEmail,
      subject: 'Reset your Science Bowl Online password',
      html
    });
    return { sent: true };
  } catch (err) {
