...
const getUserGames = asyncHandler(async (req, res) => {
  ...
  const limit = String(limitParam).toLowerCase() === 'all'
    ? 5000
    : Math.min(Number.isFinite(parsedLimit) && parsedLimit > 0 ? parsedLimit : 20, 5000);
  ...
  const [games, total] = await Promise.all([
    Game.find(playerFilter)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit)
      .select('-questions.player1Response.answer -questions.player2Response.answer'),
    Game.countDocuments(playerFilter)
  ]);
...
const getUserRatingHistory = asyncHandler(async (req, res) => {
  ...
  const days = rawDays === 'all' || rawDays === 'alltime'
    ? null
    : (parseInt(rawDays, 10) || 30);
  ...
  res.json({
    success: true,
    data: { currentRating: user.rating, peakRating: user.peakRating, history }
  });
});
...
const getLeaderboard = asyncHandler(async (req, res) => {
  ...
  switch (sortBy) {
    ...
    case 'gamesPlayed':
      ...
      break;
    case 'winStreak':
      sortField = { 'stats.longestWinStreak': -1 };
      break;
    default:
      sortField = { rating: -1 };
  }

  // Show all active users on rating leaderboard (including new users).
  const leaderboardQuery = { isActive: true };
  ...
  const [users, total] = await Promise.all([
    User.find(leaderboardQuery)
      ...
      .skip(skip)
      .limit(limit)
      .select('username displayName avatar rating stats.gamesPlayed stats.gamesWon stats.longestWinStreak'),
    User.countDocuments(leaderboardQuery)
  ]);
...
const searchUsers = asyncHandler(async (req, res) => {
  ...
  const users = await User.find({
    ...
    .limit(limit)
    .select('username displayName avatar rating');
...
const getUserStats = asyncHandler(async (req, res) => {
  ...
  const winRate = user.stats.gamesPlayed > 0
    ? Math.round((user.stats.gamesWon / user.stats.gamesPlayed) * 1000) / 10
    : 0;

  const accuracy = user.stats.questionsAnswered > 0
    ? Math.round((user.stats.questionsCorrect / user.stats.questionsAnswered) * 1000) / 10
    : 0;
