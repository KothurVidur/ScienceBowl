...
class MatchmakingHandler {
  ...
  async handleJoinQueue(socket, data = {}) {
    try {
      ...
      if (this.queue.find(p => p.userId === socket.user._id.toString())) {
        socket.emit('matchmaking:error', {
          message: 'Already in queue from another session'
        });
        return;
      }
      const player = {
  ...
  async createMatch(player1, player2) {
    try {
      ...
      if (socket1) socket1.emit('matchmaking:error', {
        message
      });
      if (socket2) socket2.emit('matchmaking:error', {
        message
      });
    }
  }
  ...
  getQueueStats() {
    ...
    return {
      ...
      averageRating: this.queue.length > 0 ? Math.round(this.queue.reduce((sum, p) => sum + p.rating, 0) / this.queue.length) : 0
    };
  }
}
